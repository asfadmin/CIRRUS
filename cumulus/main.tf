...
locals {
  ...
  protected_bucket_names = [for k, v in var.bucket_map : v.name if v.type == "protected"]
  public_bucket_names    = [for k, v in var.bucket_map : v.name if v.type == "public"]
...
resource "aws_s3_bucket_object" "bucket_map_yaml" {
  ...
  content = templatefile("${local.prefix}-internal/${local.prefix}/thin-egress-app/bucket_map.yaml.tmpl", {
    protected_buckets = local.protected_bucket_names,
    public_buckets = local.public_bucket_names
  })
  etag    = md5(templatefile("${local.prefix}-internal/${local.prefix}/thin-egress-app/bucket_map.yaml.tmpl", {
    protected_buckets = local.protected_bucket_names,
    public_buckets = local.public_bucket_names
  }))
